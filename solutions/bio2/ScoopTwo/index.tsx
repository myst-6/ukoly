...
export const ScoopTwo = () => {
  return (
    <>
      ...
      <SText>
        There exists a standard $\mathcal{brace("O")}(n)$ algorithm to find the diameter of a tree. Perform a DFS from any node, and then perform a DFS from the deepest node. The diameter of the tree is the path from the deepest node in the first DFS to the deepest node in the second DFS. So the depth of the second DFS is the length of the diameter of the tree. We can then evaluate $6E - 3d$. For a proof of this algorithm and further applications of the diameter, see <Link href="https://codeforces.com/blog/entry/101271#:~:text=A%20diameter%20of%20the%20tree,simple%20path%20a→b).">here</Link>.
      </SText>
     
      <STitle>Sample Explanation</STitle>
      <SText>
        In this case, the input is a tree with $3$ vertices and $2$ edges, with diameter $2$. The answer is thus $6 * 2 - 3 * 2 = 6$.
      </SText>
      <STitle>Code</STitle>
